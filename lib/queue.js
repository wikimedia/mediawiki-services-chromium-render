...
const { rendererErrors } = require('./renderer');
...
const callbackErrors = {
    ...
    renderTimeout: 4,
    // when the renderer got 404 error
    pageNotFound: 5,
    // when render was aborted
    abort: 6
};
...
class Queue {
     ...
     * @param {Object} metrics app metrics
     */
    constructor(queueOptions, logger, metrics) {
        ...
        data._timeoutID = setTimeout(() => {
            queue.remove((worker) => {
                if (worker.data.id === data.id) {
                    ...
                    this._metrics.increment('queue.timeout');
    ...
    push(data, callback) {
        ...
        this._metrics.gauge('queue.jobs_count', queueSize);
        ...
        if (this._isQueueFull()) {
            ...
            this._metrics.increment('queue.full');
        ...
        this._metrics.increment('queue.new');
    ...
    _worker(data, callback) {
        ...
        this._metrics.endTiming('job.wait_time', data._timeAtQueuePush);
        this._metrics.increment('job.started');
        ...
        timedRender(data, (error, pdf) => {
            ...
            if (error && error.code === 'ETIMEDOUT') {
                ...
                this._metrics.increment('job.timeout');
    ...
    _render(data, callback) {
        let renderTime;
        data.renderer
            .articleToPdf(data.uri, data.format, data.headers)
            .then((pdf) => {
                ...
                this._metrics.increment('job.successful');
                this._metrics.endTiming('job.render_time', data._timeAtRenderStart);
                callback(null, pdf);
            })
            .catch((error) => {
                if (error && error.code && error.code === rendererErrors.ABORT) {
                    // abort requests are not treated as regular failures
                    callback(callbackErrors.abort, null);
                    return;
                }
                renderTime = Date.now() - data._timeAtRenderStart;
                this._metrics.increment('job.failed');

                switch (error.code) {
                    case rendererErrors.NOT_FOUND:
                        this._logger.log('info/render', {
                            msg: 'Render failed. Page not found.',
                            id: data.id
                        });
                        this._metrics.increment('job.failed.error.not_found');
                        callback(callbackErrors.pageNotFound, null);
                        break;
                    case rendererErrors.MALFORMED_RESPONSE:
                        // lets log those errors, by renderTime we will find if this is related
                        // to the NavigationTimeout issue, by aborted we will find if this is
                        // related to aborted requests.
                        this._logger.log('warn/render', {
                            msg: 'Puppeteer returned malformed response.',
                            renderTime,
                            aborted: data.renderer.isAborted(),
                            id: data.id
                        });
                        callback(callbackErrors.renderFailed, null);
                        break;
                    case rendererErrors.SERVER_ERROR:
                        this._logger.log('error/render', {
                            msg: 'Render failed. Renderer retrieved an error page.',
                            id: data.id,
                            renderTime,
                            responseCode: error.responseCode
                        });
                        this._metrics.increment('job.failed.error.server_error');
                        callback(callbackErrors.renderFailed, null);
                        break;
                    default:
                        this._logger.log('error/render', {
                            msg: 'Render failed. Unknown reason',
                            id: data.id,
                            renderTime,
                            error: error.message
                        });
                        this._metrics.increment('job.failed.error.unknown');
                        callback(callbackErrors.renderFailed, null);
                }
            });
    }
    ...
    abort(data) {
        ...
        this._queueObject.remove((worker) => {
            if (worker.data.id === data.id) {
                ...
                this._metrics.increment('queue.abort');
                this._metrics.endTiming('queue.abort.time', data._timeAtQueuePush);
                taskStarted = false;
                return true;
            }
            return false;
        });
        // Important: The queue allows only to remove queued items (the one that did
        // not start processing). After task got into rendering phase, queue internally
        // moves the job from from queue._tasks to queue._workerList.
        // The queue._workerList is internal and cannot be reached in any way.
        //
        // The renderer itself has to stop and reject the job, otherwise it will
        // keep the spot in the queue.
        if (taskStarted) {
            ...
            this._metrics.increment('job.abort');
            this._metrics.endTiming('job.abort.time', data._timeAtRenderStart);
            data.renderer.abortRender();
        }
    }
