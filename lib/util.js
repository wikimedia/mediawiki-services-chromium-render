            ...
            layer.handle = function(req, res, next) {
                ...
                .finally(() => {
                    ...
                    app.metrics.makeMetric({
                        ...
                        prometheus: {
                            name: 'proton_router_request_duration_seconds',
                    ...
                    }).observe(Date.now() - startTime, [path || 'root', req.method, statusCode]);
                    app.metrics.makeMetric({
                        type: 'Counter',
                        name: 'router.count',
                        prometheus: {
                            name: 'proton_router_requests_count',
                            help: 'requests count handled by router'
                        },
                        labels: {
                            names: ['path', 'method', 'status'],
                            omitLabelNames: true
                        }
                    }).increment(1, [path || 'root', req.method, statusCode]);
                });
            };
        });
    });
...
function setErrorHandler(app) {
    ...
    app.use((err, req, res, next) => {
        ...
        if (err.constructor === HTTPError) {
            ...
            // is this an HTTPError defined elsewhere? (preq, express)
            if (err.status) {
...
/**
 * Returns downloadable PDF Content-Disposition header from title
 * @param {string} title
 */
