...
class Renderer {
     ...
     */
    constructor(puppeteerOptions, pdfOptions, userAgent, isMobile, logger) {
    ...
    articleToPdf(url, format, headers) {
        ...
        return new BBPromise((resolve, reject) => {
            ...
            .then((response) => {
                ...
                if (!response) {
                    const err = new Error(`Got undefined response for request to ${url}`);
                    err.code = rendererErrors.MALFORMED_RESPONSE;
                    throw err;
                } else if (!response.ok()) {
